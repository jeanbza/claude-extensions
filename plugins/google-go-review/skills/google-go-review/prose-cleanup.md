...
- **Signature echo.** A doc comment that names the parameters and return values
  in prose and adds nothing else. `// Add adds a and b and returns the sum.` on
  `func Add(a, b int) int` earns its keep only if there is something to say
  about overflow, units, or nil handling.
...
- **Type restatement.** `// userID is a string` next to `userID string`.
...
- Doc comments on exported identifiers. Go requires them for anything a user of
  the package sees; make them say something rather than deleting them. They
  start with the identifier's name and form a complete sentence, per
  `references/style-decisions` and Effective Go's Commentary section.

When in doubt, ask what a reader who understands Go but not this change needs.
...
| Instead of                                       | Write                                  |
| ------------------------------------------------ | -------------------------------------- |
| `// This function is responsible for parsing X`  | `// parse reads X.` or nothing         |
...
| `// We'll use a map for O(1) lookups`            | `// Map, not slice: lookups are hot.`  |
